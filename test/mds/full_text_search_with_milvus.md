...
我们已经学习了如何在 LangChain 和 Milvus 中使用基本的 BM25 内置功能。下面让我们介绍一种使用混合检索和重排的优化 RAG 实现。
该图显示了混合检索和重排过程，将用于关键词匹配的 BM25 和用于语义检索的向量搜索结合在一起。来自这两种方法的结果经过合并、Rerankers 和传递给 LLM 生成最终答案。

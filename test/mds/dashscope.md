...
本主题介绍如何在 Milvus 中配置和使用 DashScope 嵌入功能。
选择嵌入模型
以下是当前可用的 DashScope 嵌入模型，供快速参考：
模型名称
尺寸
每行最大标记数
支持的语言
...
告诉 Milvus 在调用 DashScope 时使用哪个密钥
...
这样，Milvus 向 DashScope Embeddings 端点发送的每个请求都会绑定特定密钥。
选项 2：环境变量
...
是
使 DashScope 密钥在每个 Milvus 容器中可用
（当
milvus.yaml
中存在 DashScope 密钥时，忽略
该变量）
。
在你的
docker-compose.yaml
...
"dashscope"
,
# Embedding provider name (must be "dashscope")
"model_name"
:
"text-embedding-v3"
,

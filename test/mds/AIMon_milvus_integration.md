利用 AIMon 和 Milvus 提高法律硕士申请的检索质量
概述
在本教程中，我们将帮助您构建一个检索增强生成（RAG）聊天机器人，回答关于
会议库数据集的
问题。
在本教程中，您将学会
构建一个 LLM 应用程序，回答用户与会议库数据集相关的问题。
定义并衡量 LLM 应用程序的质量。
使用 2 种增量方法提高应用程序的质量：使用混合搜索的向量数据库和最先进的重排序器。
技术栈
向量数据库
在此应用中，我们将使用
Milvus
管理和搜索大规模非结构化数据，如文本、图像和视频。
LLM 框架
LlamaIndex 是一个开源数据协调框架，通过促进私有数据与 LLMs 的整合，简化了大型语言模型（LLM）应用的构建，通过检索-增强生成（RAG）管道实现了上下文增强生成式人工智能应用。我们将在本教程中使用 LlamaIndex，因为它具有很高的灵活性和更好的低级应用程序接口抽象。
LLM 输出质量评估
AIMon
为幻觉、上下文质量问题、LLMs 的指令遵从性、检索质量和其他 LLM 可靠性任务提供了专有的 Judge 模型。我们将使用 AIMon 来判断 LLM 应用程序的质量。
$
...
AIMon 账户
。
将此秘密添加到 Colab Secrets（左侧面板上的 "key "符号）
如果您在其他非谷歌 Colab 环境中，请自行替换谷歌 Colab 相关代码
AIMON_API_KEY
在此处
注册
OpenAI 账户
，并在 Colab secrets 中添加以下密钥：
OPENAI_API_KEY
所需 API 密钥
import
os
...
我们将使用
MeetingBank
数据集，该数据集是从美国 6 个主要城市的市议会创建的基准数据集，用于补充现有数据集。它包含 1,366 次会议，超过 3,579 小时的视频，以及文字记录、会议记录 PDF 文档、议程和其他元数据。
为了这次练习，我们创建了一个较小的数据集。请
点击此处
。
...
folder_path =
"/content/meetingbank_train_split.hf"
if
...
id
=1bs4kwwiD30DUeCjuqEdOeixCuI-3i9F5
$
gdown https://drive.google.com/uc?
id
=1fkxaS8eltjfkzws5BRXpVXnxl2Qxwy5F
Downloading...
...
tar_file_path =
"/content/meetingbank_train_split.tar.gz"
extract_path =
"/content/"
# Extract the file
with
...
train_split = load_from_disk(extract_path +
"meetingbank_train_split.hf"
)
...
print
(
f"Total number of tokens in train split:
{total_tokens}
"
)
...
3864.124031007752
分析
我们有 258 份记录誊本，所有这些记录誊本共有约 100 万个标记。每份记录誊本平均有 3864 个标记。
指标
数值
记录誊本数量
258
...
"/content/score_metrics_relevant_examples_2.csv"
)
len
(queries_df[
"Query"
...
指标定义
此质量得分指标将帮助我们了解 LLM 对上述查询的响应程度。为衡量应用程序的质量，我们将运行一组查询，并汇总所有这些查询的质量得分。
LLM 应用程序质量得分是 AIMon 3 个单独质量指标的组合：
幻觉得分
（hall_score）：检查生成的文本是否符合所提供的上下文。接近 1.0 的分数表示有强烈的幻觉迹象，接近 0.0 的分数表示幻觉迹象较少。因此，在计算最终质量得分时，我们将使用（1.0-hall_score）。
指示遵守得分
（ia_score）：检查法律硕士是否遵守了所有明确的指示。ia_score 越高，说明遵守情况越好。分数越低，说明遵守情况越差。
检索相关性得分
（rr_score）：检查检索到的文档是否与查询相关。得分接近 100.0 表示文档与查询完全相关，得分接近 0.0 表示文档与查询相关性较差。
...
aimon_response
    ...
    retrieval_rel_scores = aimon_response.detect_response.retrieval_relevance[
0
][
"relevance_scores"
]
    avg_retrieval_relevance_score = (
        statistics.mean(retrieval_rel_scores)
if
len
(retrieval_rel_scores) >
0
else
0.0
)
    hall_score = aimon_response.detect_response.hallucination[
"score"
]
    ia_score = aimon_response.detect_response.instruction_adherence[
"score"
]
return
(
...
True
,
# This publishes results to the AIMon UI
application_name=
"meeting_bot_app"
,
    model_name=
...
1.简单、粗暴的方法
在第一种简单的方法中，我们将使用列文士坦距离（Levenshtein Distance）将文档与给定的查询进行匹配。匹配度最高的前 3 个文档将被发送到 LLM，作为回答问题的上下文。
注意：该单元的执行时间约为 3 分钟。
等待期间，请享用您最喜欢的饮料:)
from
fuzzywuzzy
...
@detect
def
get_fuzzy_match_response
(
query, docs
):
    response = query_openai_with_context(query, docs)
return
docs, query, instructions_to_evaluate, response, task_definition
...
print
(
"Avg. Retrieval relevance score across chunks: {} for query: {}"
.
format
(
...
print
(
"Average Quality score for brute force approach: {}"
.
format
...
print
(
"Average retrieval relevance score for brute force approach: {}"
.
format
(
...
AIMon
计算的幻觉分数等单项指标。
2.使用 VectorDB（Milvus）进行文档检索
现在，我们将通过加入向量 DB 来提高质量得分。与之前的方法相比，这也有助于改善查询延迟。
我们需要注意两个主要部分：摄取和基于 RAG 的问答。摄取管道处理来自 Meeting Bank 数据集的记录誊本，并将其存储到 Milvus 向量数据库中。RAG 问答管道在处理用户查询时，首先会从向量存储中检索相关文档。然后，这些文档将被用作 LLM 生成其回复的基础文档。我们利用 AIMon 来计算质量得分，并持续监控应用程序的
幻觉
、
指令遵守
情况和
...
:
            ...
            doc = Document(text=clean_text, metadata={
"index"
: indx})
...
print
(
f"Failed to process transcript number
{indx}
:
{
str
...
documents = extract_and_create_documents(train_split[
"transcript"
])
设置一个基于开放人工智能的嵌入计算模型。
from
...
在这个单元中，我们为
documents
计算嵌入式，并将其索引到 MilvusVectorStore 中。
from
llama_index.core
import
...
现在，VectorDB 索引已经建立，我们将利用它来回答用户查询。在下面的单元格中，我们将创建一个检索器，设置 LLM，并建立一个 LLamaIndex 查询引擎，该引擎可与检索器和 LLM 接口，以回答用户的问题。
from
...
至此，查询引擎、检索器和 LLM 都已安装完毕。接下来，我们设置 AIMon 来帮助我们测量质量分数。我们使用的是在上述单元格中创建的
@detect
装饰器。
ask_and_validate
中唯一的附加代码是帮助 AIMon 与 LLamaIndex 检索到的文档 "节点 "接口。
import
logging
@detect
def
ask_and_validate
(
user_query, user_instructions, query_engine=query_engine
):
...
:
                    logging.info(
"Node does not have required attributes."
)
else
:
            logging.info(
"No source_nodes attribute found in the chat response."
)
return
contexts, relevance_scores

    context, relevance_scores = get_source_docs(response)
return
context, user_query, user_instructions, response.response, task_definition
...
让我们通过
queries_df
中的 LlamaIndex 查询引擎运行所有查询，并使用 AIMon 计算总体质量得分。
注意：大约需要 2 分钟
import
time
...
print
(
"Average Quality score for vector DB approach: {}"
.
format
...
print
(
"Average retrieval relevance score for vector DB approach: {}"
.
format
(
...
🎉质量得分提高！
请注意，使用基于 RAG 的质量保证系统后，所有查询的总体质量得分都有所提高。
3.在检索中添加重新排序
现在，我们将使用 AIMon 的 LlamaIndex
后处理器重排序集成
添加 AIMon 的
域适应性重排序器
。
如下图所示，Rerankers 通过使用更先进的查询-文档匹配功能，帮助将最相关的文档挤到最前面。AIMon 重新排序器的独特之处在于可以根据领域进行定制。与 LLM 的提示工程类似，您可以使用
task_definition
字段自定义每个域的 Reranker 性能。这种最先进的 Reranker 能以超低的亚秒级延迟（对于约 2k 上下文）运行，其性能在 MTEB Reranking 排行榜上名列前五。
...
让我们再次运行查询并重新计算总体质量得分，看看是否有改进。
✨
AIMon 的重新排序应该不会增加额外的延迟开销，因为它实际上减少了需要发送到 LLM 以生成响应的上下文文档数量，从而在网络 I/O 和 LLM 令牌处理成本（金钱和时间）方面提高了操作效率。
注：此步骤需要 2 分钟
import
time
...
请注意使用 Reranker 和不使用 Reranker 以及使用简单粗暴方法时文档相关性平均得分的差异。
# This is the average quality score.
avg_quality_score_rr = statistics.mean(qual_scores_rr)
print
(
"Average Quality score for AIMon Re-ranking approach: {}"
.
format
(
...
print
(
"Average retrieval relevance score for AIMon Re-ranking approach: {}"
.
format
(
...
质量得分再次提高！
请注意，使用 AIMon 的 Reranker 后，所有查询的总体质量得分都有所提高。
总之，如下图所示，我们证明了以下几点：
使用 3 个不同质量指标的加权组合计算质量得分：幻觉得分、指令遵循得分和检索相关性得分。
使用暴力字符串匹配法建立质量基线，将文档与查询进行匹配，并将其传递给 LLM。
使用向量数据库（此处使用 Milvus）提高基线质量
使用 AIMon 的低延迟、领域适应性再排序器进一步提高质量得分。
我们还展示了通过添加 AIMon 的重新排序器如何显著提高检索相关性。
我们鼓励您尝试使用本笔记本中展示的不同组件来
进一步提高质量得分
。其中一个方法是使用
instructions
字段在上面的指令一致性检测器中添加自己的质量定义。另一种方法是添加
AIMon 的另一个检测器模型
，作为质量度量计算的一部分。
import
pandas
...
"Increase in Quality Score (%)"
    ...
    (df_scores[
"Quality Score"
] - avg_quality_score_bf) / avg_quality_score_bf
) *
100
df_scores.loc[
0
...
44.195372
177.940153
上表总结了我们的结果。实际数字会因各种因素而变化，如 LLM 响应质量的变化、VectorDB 中近邻搜索的性能等。
总之，如下图所示，我们对您的 LLM 应用程序的质量得分、RAG 相关性和指令跟踪能力进行了评估。我们使用 AIMon 的重新排序器提高了应用程序的整体质量以及从您的 RAG 中检索到的文档的平均相关性。
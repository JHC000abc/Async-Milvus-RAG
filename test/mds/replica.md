...
关键概念
内存复制以复制组的形式组织。每个副本组都包含
分片
副本。每个分片副本都有一个流副本和一个历史副本，分别对应于分片（即 DML 通道）中不断增长和封存的
分段
。
内存复制工作原理示例
副本组
...
组成，负责处理历史数据和副本。
分片副本
分片副本由流式副本和历史副本组成，二者属于同一个
分片
。副本组中碎片副本的数量由指定 Collections 中碎片的数量决定。
流副本
流副本包含来自同一 DML 通道的所有
增长分段
。从技术上讲，流副本只能由一个副本中的一个查询节点提供服务。
历史副本
